...
├── README.md               # This file
├── include/                # Header files (.hpp, .cuh)
│   ├── evmcpp/             # CPU Namespace directory
│   │   ├── laplacian_pyramid.hpp
│   │   ├── processing.hpp
│   │   ├── butterworth.hpp
│   │   └── gaussian_pyramid.hpp
│   └── evmcuda/            # CUDA Namespace directory
│       ├── color_conversion.cuh
│       └── temporal_filter.cuh
├── src/                    # C++ source files (.cpp)
│   ├── main.cpp            # Main application entry point & pipeline
│   ├── evmcpp/             # CPU Implementation files matching headers
│   │   ├── laplacian_pyramid.cpp
│   │   ├── processing.cpp
│   │   ├── butterworth.cpp
│   │   └── gaussian_pyramid.cpp
│   └── evmcuda/            # CUDA Implementation files (.cu)
│       ├── color_conversion.cu
│       └── temporal_filter.cu
├── tests/                  # Unit tests
...
│   ├── test_gaussian.cpp
│   ├── test_cuda_color.cpp     # CUDA Tests
│   └── test_cuda_temporal.cpp  # CUDA Tests
...
*   **`include/evmcpp/*.hpp` & `src/evmcpp/*.cpp`:** These implement the core logic, mirroring the Python structure:
    ...
    *   `gaussian_pyramid`: Contains CPU implementations `spatiallyFilterGaussian` (using custom `pyrDown`/`pyrUp`), `temporalFilterGaussianBatch` (using FFT-based ideal bandpass filter via `cv::dft`/`cv::idft`), and `reconstructGaussianFrame`.
    *   `evmcuda/color_conversion`: Contains CUDA kernels (`rgb2yiqKernel`, `yiq2rgbKernel`) and C++ wrappers (`rgb2yiq_gpu`, `yiq2rgb_gpu`) for GPU-accelerated color space conversion using raw device pointers. Verified against CPU implementation.
    *   `evmcuda/temporal_filter`: Contains CUDA kernels and wrappers for temporal filtering:
        *   `temporalFilterGaussianBatch_gpu`: Uses cuFFT and a custom kernel (`applyMaskAndAmplifyKernelR2C`) for FFT-based filtering and amplification. Verified against CPU implementation.
        *   `filterLaplacianLevelFrame_gpu`: Implements single-frame IIR bandpass filtering using `iirTemporalFilterFrameBandpassKernel`. Includes spatial alpha attenuation logic. Partially verified (kernel runs but numerical comparison fails due to complexity in matching CPU reference exactly). Further work postponed.
*   **`src/main.cpp`:**
...
*   **CPU Color Conversion:** `rgb2yiq`, `yiq2rgb` (using `cv::transform`).
*   **CPU Pyramid Operations:** Custom `pyrDown`, `pyrUp` (using `cv::filter2D`).
*   **CPU Laplacian Pathway:** `generateLaplacianPyramid`, `getLaplacianPyramids`, `filterLaplacianPyramids` (using IIR Butterworth filter), `reconstructLaplacianImage`.
*   **CPU Gaussian Pathway:** `spatiallyFilterGaussian`, `temporalFilterGaussianBatch` (using FFT filter), `reconstructGaussianFrame`.
*   **CPU Butterworth Coefficients:** `calculateButterworthCoeffs` (1st order low-pass).
*   **CUDA Color Conversion:** `rgb2yiq_gpu`, `yiq2rgb_gpu` (using custom kernels).
*   **CUDA Gaussian Temporal Filter:** `temporalFilterGaussianBatch_gpu` (using cuFFT and custom kernel).
*   **CUDA Laplacian Temporal Filter:** `filterLaplacianLevelFrame_gpu` (partially implemented/verified IIR filter).
*   **Constants:** Gaussian kernel, RGB/YIQ matrices (CPU and CUDA `__constant__`).
*   **Pipeline:** CPU pipeline orchestrated in `main.cpp` and `processing.cpp`. CUDA pipeline integration pending.
...
*   **CUDA Acceleration:**
    *   **Completed:**
        * Color Conversion (`rgb2yiq_gpu`, `yiq2rgb_gpu`)
        * Gaussian Temporal Filter (`temporalFilterGaussianBatch_gpu`)
        * **Laplacian Temporal Filter (`filterLaplacianLevelFrame_gpu`)** — now fully implemented and verified across multiple frames with correct state propagation.
    *   **Abandoned (Custom Kernels):** Pyramid Operations (`pyrDown_gpu`, `pyrUp_gpu`) - recommend using `cv::cuda::pyrDown`/`pyrUp` if needed.
    *   **Remaining:** Reconstruction kernels (Gaussian/Laplacian), Integration into main pipeline (`main.cpp`). Consider using NPP for further optimization if desired.
