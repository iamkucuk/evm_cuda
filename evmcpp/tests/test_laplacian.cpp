...
#include "evmcpp/laplacian_pyramid.hpp" // Header for the function under test
#include "evmcpp/processing.hpp"       // Needed for types, maybe setup
...
TEST_F(LaplacianPyramidTest, GenerateLaplacianPyramidNumerical) {
    ...
    ASSERT_NO_THROW(laplacian_result = evmcpp::generateLaplacianPyramid(yiq_frame0, num_levels, kernel));
...
TEST_F(LaplacianPyramidTest, GetLaplacianPyramidsBatch) {
    ...
    ASSERT_NO_THROW(laplacian_result_batch = evmcpp::getLaplacianPyramids(rgb_frames, num_levels, kernel));
