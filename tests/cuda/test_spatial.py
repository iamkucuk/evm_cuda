"""Spatial primitives tests — corr_dn and up_conv vs vidmag.cpu.pyramids."""
...
from vidmag.cpu.pyramids import corr_dn_axis, up_conv_axis, BINOM5
...
def test_corr_dn_rows_matches_baseline(h, w):
    ...
    got = _vidmag_cuda.corr_dn_rows(img, BINOM5.astype(np.float32))
...
def test_corr_dn_cols_matches_baseline(h, w):
    ...
    got = _vidmag_cuda.corr_dn_cols(img, BINOM5.astype(np.float32))
    assert got.shape == expected.shape
    assert abs_err(got, expected) < TOL["corr_dn"]
