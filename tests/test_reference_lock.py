...
* :data:`vidmag.cpu.magnify.DROP_LAST` and :data:`vidmag.cpu.magnify.EXAGGERATION_FACTOR`
  — the two magic numbers hardcoded in the four MATLAB amplification scripts.
* :data:`vidmag.cpu.pyramids.BINOM5` / :data:`vidmag.cpu.pyramids.BINOM5_SUM1` — the
...
from vidmag.cpu.magnify import DROP_LAST, EXAGGERATION_FACTOR
...
def _read_tol_from_conftest() -> dict[str, float]:
    ...
    ``vidmag.cuda._vidmag_cuda`` extension and builds pytest markers from the
