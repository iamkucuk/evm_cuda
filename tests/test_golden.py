...
pipelines take a *path* and decode it themselves (``vidmag.cpu.magnify._read_frames``
-> ``cv2.VideoCapture``), so a file has to exist. The project's own writer
(:func:`vidmag.io.video.encode_video`) is H.264 ``yuv420p`` at ``crf 18`` — lossy and
...
def write_lossless_video(path: str | Path, frames_bgr: np.ndarray, fps: float) -> None:
    ...
    Deliberately *not* :func:`vidmag.io.video.encode_video` — see the module
