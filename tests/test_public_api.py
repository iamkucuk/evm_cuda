...
# Everything importable as ``from vidmag import <name>``. Grouped by what it is
...
EXPECTED_ROOT = {
    ...
    # because that makes `vidmag.api.magnify` a path callers can rely on, which is
    ...
    # documented as `from vidmag.stream import MotionStream`, so it is a path
    # callers may rely on.
    "stream",
    # Metadata.
    "__version__",
...
def test_every_promised_name_is_importable():
    missing = sorted(n for n in EXPECTED_ROOT if not hasattr(vidmag, n))
    assert not missing, (
...
def test_nothing_extra_leaked_into_the_public_surface():
    ...
    actual = {n for n in dir(vidmag) if not n.startswith("_")} | {"__version__"}
...
def test_all_matches_what_is_actually_exported():
    declared = set(vidmag.__all__)
    missing = sorted(n for n in declared if not hasattr(vidmag, n))
...
def test_importing_evm_does_not_load_the_gpu_extension():
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys, vidmag; print('vidmag.cuda' in sys.modules)",
        ],
        capture_output=True,
        text=True,
        check=True,
    ...
    assert result.stdout.strip() == "False", (
        "importing vidmag eagerly imported vidmag.cuda; on a machine with no GPU "
...
def test_the_gpu_operations_module_is_named_even_without_a_gpu():
    """``vidmag.cuda.ops`` must be a promised name regardless of hardware.
