...
# src/evm/cuda/CMakeLists.txt or the Makefile.
bash scripts/dev/verify_install.sh
```

**`pip install .` works on every machine, and `PYTHONPATH` is gone from the repo.**
`src/evm/cuda/CMakeLists.txt` runs `check_language(CUDA)` before anything else: with no nvcc it
...
rule from `src/evm/cuda/DESIGN.md` is
...
`src/evm/cuda/kernels/*.cu` beside `src/evm/opencl/kernels.cl`,
`src/evm/metal/kernels.metal` and `src/evm/vulkan/shaders/*.comp`. One place to look,
...
time, so those 30 files do ship. Verified after the move: the wheel contains zero
`.cu`, `.cuh`, `.cpp` or `CMakeLists.txt` entries, and the sdist contains all 14.

**The NVIDIA GPU code is the primary optimisation target; the other backends follow it.**
Performance work starts in `src/evm/cuda/kernels/*.cu`. Once a change there is accepted, check whether
...
5. **`DROP_LAST = 10` is applied inside the frame readers, not at the API boundary**:
   ...
   `src/evm/cuda/include/evm_common.cuh:21`). Any array-in API must decide this explicitly — see
...
| `src/evm/cuda/DESIGN.md` | Kernel map, per-stage tolerances, precision and memory-layout rationale |
