...
__global__ void corr_dn_2d_batched_kernel(
    const In* __restrict__ in,
    Out* __restrict__ out,
    int H, int W, const float* filt, int filt_len,
    int slice_stride_in, int slice_stride_out, int B)
...
#pragma unroll
    for (int ky = 0; ky < 5; ++ky) {
        const int ry = reflect1(cy + (ky - pad), H);
        const float fy = f[ky];
#pragma unroll
        for (int kx = 0; kx < 5; ++kx) {
            const int rx = reflect1(cx + (kx - pad), W);
            acc += fy * f[kx] * cvt_in<In>(sin[ry * W + rx]);
        }
    }
    out[b * slice_stride_out + yo * Wo + xo] = cvt_out<Out>(acc);
}
...
template <typename In, typename Out>
__launch_bounds__(1024, 2)
__global__ void up_conv_2d_batched_kernel(
    const In* __restrict__ in,
    Out* __restrict__ out,
    int in_H, int in_W, int out_H, int out_W,
    const float* filt, int filt_len,
    int slice_stride_in, int slice_stride_out, int B)
{
    ...
    const int yo = blockIdx.y * blockDim.y + threadIdx.y;
    const int b  = blockIdx.z;
    if (xo >= out_W || yo >= out_H || b >= B) return;
...
#pragma unroll
        for (int kx = 0; kx < 5; ++kx) {
            ...
            if (rx & 1) continue;
            const int sx = rx / 2;
            acc += fy * f[kx] * cvt_in<In>(sin[sy * in_W + sx]);
        }
    }
    out[b * slice_stride_out + yo * out_W + xo] = cvt_out<Out>(acc);
}
...
void launch_corr_dn_2d_batched(const float* in, float* out,
    ...
    const int Wo = (W + 1) / 2;
    dim3 block(32, 32, 1);
    dim3 grid(div_up(Wo, 32), div_up(Ho, 32), B);
    corr_dn_2d_batched_kernel<float, float><<<grid, block, 0, stream>>>(
        in, out, H, W, filt, filt_len, stride_in, stride_out, B);
}
...
void launch_up_conv_2d_batched(const float* in, float* out,
                               ...
                               cudaStream_t stream) {
    dim3 block(32, 32, 1);
    dim3 grid(div_up(out_W, 32), div_up(out_H, 32), B);
    up_conv_2d_batched_kernel<float, float><<<grid, block, 0, stream>>>(
        in, out, in_H, in_W, out_H, out_W, filt, filt_len,
        stride_in, stride_out, B);
}
...
void launch_corr_dn_2d_batched_f16(const __half* in, __half* out,
    ...
    dim3 block(32, 32, 1);
    dim3 grid(div_up(Wo, 32), div_up(Ho, 32), B);
    corr_dn_2d_batched_kernel<__half, __half><<<grid, block, 0, stream>>>(
        in, out, H, W, filt, filt_len, stride_in, stride_out, B);
}
...
void launch_up_conv_2d_batched_f16(const __half* in, __half* out,
    ...
    dim3 block(32, 32, 1);
    dim3 grid(div_up(out_W, 32), div_up(out_H, 32), B);
    up_conv_2d_batched_kernel<__half, __half><<<grid, block, 0, stream>>>(
        in, out, in_H, in_W, out_H, out_W, filt, filt_len,
        stride_in, stride_out, B);
}
