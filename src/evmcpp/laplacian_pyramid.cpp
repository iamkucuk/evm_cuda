#include "evmcpp/laplacian_pyramid.hpp"   // Define functions for this file
#include "evmcpp/processing.hpp"          // Includes butterworth, gaussian_pyramid
#include "evmcpp/butterworth.hpp"         // Explicitly include for Laplacian path
...
namespace evmcpp {
        ...
        const cv::Mat& kernel)
    {
        ...
        for (int i = 0; i < level; ++i) {
            ...
            // Call OpenCV's pyrDown
            cv::pyrDown(prev_image, downsampled_image); // Corrected call (output is 2nd arg, no kernel)
            if (downsampled_image.empty() && !prev_image.empty()) { // Use &&
            ...
            cv::Mat upsampled_image;
             // Call OpenCV's pyrUp
            cv::pyrUp(downsampled_image, upsampled_image, prev_image.size()); // Corrected call (output is 2nd arg, no kernel)
             if (upsampled_image.empty() && !downsampled_image.empty()) { // Use &&
                 throw std::runtime_error("pyrUp resulted in empty image at level " + std::to_string(i));
            }
             // Ensure type and size match for subtraction
             if (upsampled_image.size() != prev_image.size() || upsampled_image.type() != prev_image.type()) {
                 cv::Mat temp;
                 upsampled_image.convertTo(temp, prev_image.type()); // Match type
                 if (temp.size() != prev_image.size()) {
                    cv::resize(temp, upsampled_image, prev_image.size()); // Match size
                 } else {
            ...
            cv::subtract(prev_image, upsampled_image, laplacian_level); // laplacian = prev - upsampled
            ...
            prev_image = downsampled_image; // Prepare for next iteration

             // Break if downsampling produces an empty or 1x1 image (cannot downsample further)
        ...
        LOG("generateLaplacianPyramid: Output pyramid size=" + std::to_string(laplacian_pyramid.size()));
        ...
        const cv::Mat& kernel)
    {
        ...
        // If pyramid has less than 2 levels, only the base YIQ conversion happens.
        if (num_levels < 2) {
             ...
             // Convert the initial YIQ back to RGB and return
             ...
             return reconstructed_rgb_uint8; // Return early
        } else {
            for (size_t level = 1; level < num_levels; ++level) {
                ...
                if (current_level_filtered.type() != CV_32FC3) {
                     cv::Mat temp;
                     if (current_level_filtered.channels() == 3 && current_level_filtered.depth() != CV_32F) { // Use &&
                ...
                for (size_t up_iter = 0; up_iter < level; ++up_iter) {
                    ...
                    // Ensure target level is valid before getting size
                    ...
                    cv::pyrUp(upsampled_level, upsampled_level, target_size); // Corrected call (output is 2nd arg, no kernel)
        ...
        cv::Mat reconstructed_rgb_uint8;
        // Use saturate_cast for efficient clipping and conversion
        reconstructed_rgb_float.convertTo(reconstructed_rgb_uint8, CV_8UC3); // saturate_cast handles clipping
