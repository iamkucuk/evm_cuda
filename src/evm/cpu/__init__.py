...
* :mod:`evm.cpu.pyramids` — matlabPyrTools ``binom5`` + ``reflect1`` corrDn /
  upConv, and the Laplacian pyramid built on top of them.
* :mod:`evm.cpu.filters` — the three temporal filters (ideal FFT, first-order
  Butterworth, direct r1/r2 IIR).
* :mod:`evm.cpu.magnify` — the four ``magnify_*`` pipelines, one per MATLAB
  ``amplify_spatial_*`` script.

Everything re-exported here is also re-exported from the package root
(``evm``), except :func:`corr_dn_axis` and :func:`up_conv_axis`, which are the
