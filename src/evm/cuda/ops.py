"""The GPU building blocks, as operations on :class:`~evm.cuda.array.DeviceArray`.
...
Each function here has a counterpart of the same name in :mod:`evm.cpu.ops`
...
def bgr_u8_to_ntsc(frames: DeviceArray) -> DeviceArray:
    ...
    _evm_cuda.batched_bgr_u8_to_ntsc_f32(frames.ptr, out.ptr, T, H, W)
    return out
...
def build_lpyr(frames: DeviceArray, levels: int) -> list[DeviceArray]:
    ...
    planar = DeviceArray.empty((T * 3, H, W), np.float32)
    _evm_cuda.batched_to_planar_3ch_chan_outer(frames.ptr, planar.ptr, T, H, W)
    ...
    for h, w in sizes:
        ...
        _evm_cuda.copy_f32(flat.ptr, band.ptr, count, src_offset=offset)
        bands.append(band)
        offset += count
    return bands
...
def recon_lpyr(bands: list[DeviceArray], height: int, width: int) -> DeviceArray:
    ...
    for b in bands:
        ...
        _evm_cuda.copy_f32(b.ptr, flat.ptr, count, dst_offset=offset)
        offset += count
...
def _to_signal(frames: DeviceArray) -> tuple[DeviceArray, int, int]:
    ...
    _evm_cuda.batched_thwc_to_nt(frames.ptr, sig.ptr, T, N)
    return sig, T, N
...
def _to_frames(sig: DeviceArray, shape: tuple[int, ...]) -> DeviceArray:
    ...
    _evm_cuda.batched_nt_to_thwc_scaled(sig.ptr, out.ptr, T, H * W * C, 1.0)
    return out
...
def ideal_bandpass(
    ...
    returns zeros; :func:`evm.cpu.filters.ideal_bandpass` warns when that
...
def iir_bandpass(frames: DeviceArray, r1: float, r2: float) -> DeviceArray:
    ...
    sig, T, N = _to_signal(frames)
    filtered = DeviceArray.empty((N, T), np.float32)
    _evm_cuda.batched_iir_bandpass(sig.ptr, filtered.ptr, T, N, r1, r2)
    return _to_frames(filtered, frames.shape)
