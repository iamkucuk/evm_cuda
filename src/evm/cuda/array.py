...
class DeviceArray:
    ...
    :mod:`evm.cuda.ops`. Instances own a reference to their memory; the memory
    ...
    def from_numpy(cls, host: np.ndarray) -> "DeviceArray":
        ...
        return cls(_evm_cuda.DeviceBuffer(host), host.shape, dtype)
    ...
    def empty(cls, shape: tuple[int, ...], dtype: Any) -> "DeviceArray":
        ...
        return cls(_evm_cuda.DeviceBuffer(nbytes), shape, dtype)
    ...
    def __dlpack_device__(self) -> tuple[int, int]:
        ...
        return (_KDLCUDA, _evm_cuda.current_device())
    ...
    def __dlpack__(self, stream: Any = None) -> Any:
        ...
        return _evm_cuda.dlpack_capsule(self._buf, list(self._shape), code, bits)
...
class _ImportedTensor:
    ...
    def __del__(self) -> None:  # pragma: no cover - runs at collection time
        if self._managed:
            _evm_cuda.dlpack_release(self._managed)
            self._managed = 0
