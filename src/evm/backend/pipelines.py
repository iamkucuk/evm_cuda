...
:class:`evm.backend.Ops` serves every backend. A backend overrides it only when
...
:mod:`evm.cpu.magnify` (the correctness oracle) and :mod:`evm.cuda.batched` /
:mod:`evm.cuda.pipelines` (the fused override) and match them exactly.

An implementation is any object carrying the four functions — in practice a
module: ``evm.cpu.magnify`` is the CPU backend and ``evm.cuda`` is the CUDA one,
and both are handed to :func:`evm.backend.register` as-is.

``isinstance(evm.cpu.magnify, Pipelines)`` is ``True``. ``isinstance(evm.cuda,
...
``__getattr__`` — and ``evm/cuda/__init__.py`` resolves each core lazily through
exactly that hook, so importing ``evm.cuda`` on a CPU-only machine never reaches
...
``evm.magnify_color_gdown_ideal`` and friends keep their exact current
...
* **frame dropping is the wrapper's** — ``DROP_LAST = 10``
  (``evm/cpu/magnify.py:59``) is applied by the path-based reader. Plan decision
  D8: the path functions keep dropping the last ten frames, the array API
  (:func:`evm.magnify`) drops none unless asked. A core never drops anything.
...
:class:`~evm.backend.Ops.Array` and the return value is the same shape and
...
``evm/cpu/magnify.py:246``, the CUDA batched pipelines' ``d_out_u8`` buffer);
...
* **extra keyword-only parameters.** The CUDA cores take an ``on_stage``
  profiling hook that ``evm/cuda/benchmark.py`` and ``tests/cuda/test_benchmark.py``
...
* **lower-precision variants**, named ``<stem>_fp16_core``. ``evm.cuda`` has
  ``color_gdown_ideal_fp16_core`` and ``motion_lpyr_iir_fp16_core``; nothing
  else does. :func:`evm.magnify` looks the variant up by name and refuses
...
class Pipelines(Protocol):
    ...
    ``evm/cpu/magnify.py`` exactly, minus the path arguments."""
    ...
    def color_gdown_ideal_core(
        ...
        *,
        ...
        ``evm/cpu/magnify.py:191``, public wrapper at ``:471``.
        """
    ...
    def motion_lpyr_ideal_core(
        ...
        *,
        ...
        ``evm/cpu/magnify.py:267``, public wrapper at ``:498``.
        """
    ...
    def motion_lpyr_butter_core(
        ...
        *,
        ...
        ``evm/cpu/magnify.py:399``, public wrapper at ``:523``.
        """
    ...
    def motion_lpyr_iir_core(
        ...
        *,
        ...
        ``evm/cpu/magnify.py:431``, public wrapper at ``:550``. ``fps`` is
