...
#include "gaussian_pyramid.hpp" // Include Gaussian pyramid header
...
int main(int argc, char* argv[]) {
    ...
    try {
        if (config.mode == "laplacian") {
            ...
            std::vector<std::vector<cv::Mat>> laplacian_pyramids = getLaplacianPyramids(original_frames, config.pyramid_levels, kernel); 
            ...
            std::vector<std::vector<cv::Mat>> filtered_pyramids = filterLaplacianPyramids(
                laplacian_pyramids,
            ...
            for (size_t i = 0; i < original_frames.size(); ++i) {
                ...
                output_frames.push_back(reconstructLaplacianImage(original_frames[i], filtered_pyramids[i], kernel)); 
                if (i > 0 && i % 50 == 0) {
