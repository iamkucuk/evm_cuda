...
This post is the answer, told as a single progressive series with a stage
table after every keep. We did **not** change the EVM math (same Laplacian
levels, same r1/r2 IIR, same Figure-6 α). We changed how memory is addressed,
how buffers live across calls, and which half of the pyramid is worth fusing.

**Headline (RTX 3090, `data/baby.mp4`, 291×960×544, 9 levels, FP32 motion
compute only):**
...
| **Total** | **~9×** mid-pipeline compute |

H2D/D2H and encode are reported honestly but were **not** the target. After
this series, transfer often **exceeds** mid-pipeline compute on file→file
runs—so product latency is a different problem (NVDEC/NVENC, async H2D).

Companion material: raw step tables in
[`progressive_gains.md`](progressive_gains.md); bound/probe log in
[`bound_analysis.md`](bound_analysis.md).
...
| Clip | `data/baby.mp4` — 291 frames, 960×544, 9 Laplacian levels |
...
**Nsight Compute GPU counters are not available under WSL2**, and on TRUBA
they were often admin-blocked (`ERR_NVGPUCTRPERM`). Saturation claims use
...
(~850–860 GB/s flat copy on the 3090 ≈ 90% of ~936 GB/s GDDR6X peak).

Absolute milliseconds move with GPU load. Ratios and *which stage collapses*
are the story. Two baseline numbers for the same post–TN+sticky code
generation appear in the record (**~377 ms** end-of-session median vs
**~407 ms** later median-of-7 re-baseline). That gap is harness/noise, not a
missing fix. Pool-and-later steps use the **~407 ms** Step 0 so one continuous
table stays consistent; the layout session’s **934 → 377** is the earlier arc.

Working rule after every real fix:

1. Stage table first — who owns the milliseconds?
2. Calibrate the roof — friendly stream must approach peak BW.
...
| Level 6 — smem fused **upsample** | **127–189 ms** | **Regressed → reverted** |
| **Production** | **~96–104 ms** | **~9×** vs series start |

Full stage collapse (pre-pool ~407 ms era → production ~100 ms era):
...
On the 3090, a flat `float` copy and an f32↔f16 round-trip both reach
**~850–860 GB/s**. The device *can* saturate bandwidth when the access pattern
is friendly.

### Isolation probes (finest level, one channel, N=522 240, T=291)
...
H100 told the same story with cleaner absolutes: NT IIR ~5.0 ms → TN IIR
**0.47 ms (~12×)**; TN IIR ≈ TN copy ≈ flat stream (~77% of 3350 GB/s).
...
| “FP64 fidelity is the first wall on 3090” | **No.** FP32 state ≈ FP64 time. |
| “Second pole / subtraction is expensive” | **No.** Single-pole ≈ full bandpass. |
...
Laplacian band storage was **already** `(T, N)` per channel. The pipeline paid
**two full transposes** to put the filter on `(N, T)`—where time is contiguous
*per thread*, but **warp lanes at fixed `t` are spaced by `T` floats (~1164 B)**.
That is uncoalesced across the warp.

Coalesced form keeps the same recurrence (one location per thread, serial `t`
in registers) but addresses **`t*N + n`**, so a warp at fixed `t` is contiguous
in `n`.
...
The win is larger than “IIR kernel alone”: we removed **both** layout
transforms that owned a large fraction of Stage C bytes (probe sandwich traffic
was ~⅔ transpose).
...
| Work | Time | Note |
|---|---:|---|
| Finest 4-kernel chain (cols+rows+up×2, M slices) | **~23 ms** | Sum of parts ≈ chain (no launch tax) |
| Full `batched_lpyr_build` (9 levels) | **~280–350 ms** | **~15×** finest chain |
| `corr_dn_*` unique-byte model | often **~50%+ of peak BW** | Nearer roof than IIR ever was |

The 5-tap kernels are **not** a 3%-of-peak coalescing disaster. Full build cost
was inflated by something else: **allocating four full-resolution scratch
buffers (`M×H×W` floats each, multi‑GB) on every `batched_lpyr_build` call**,
then freeing them—same pattern in recon (2 buffers) and blur.
...
| Stage | After TN only | After sticky | Δ |
|---|---:|---:|---:|
...
Combined with the layout fix: **934 → 377 ms compute (−60%)** on that session.
FP16 motion compute moved **709 → 452 ms (−36%)** on the same arc.

### What we learned about “theoretical limits” (so far)
...
1. **Missing both bandwidth and compute utilization** signals wrong *access
   schedule* or host work—not that the algorithm must change.
2. **Roofline AI alone misled on IIR.** Low FLOP/byte looked “memory-bound”;
   a pure strided copy with the same indexing proved the walk never reached
   the memory roof.
3. **Same recurrence, different layout** put IIR on the bandwidth roof
   (TN IIR ≈ TN copy on H100 and on clean 3090 runs).
4. **Alloc thrash can dominate kernel time** even when kernels themselves are
...
|---|---:|---:|
| NTSC | **~2.6 ms @ ~93% peak BW** | **~55–67 ms** |
| lpyr_build | **~40–45 ms** | **~160 ms** |

That gap is not color math and not 5-tap math. Sticky scratch had fixed *lpyr
internals*. The **Python pipeline stage buffers** (NTSC, planar, bands,
filtered, delta, output) still RAII’d `cudaMalloc` / `cudaFree` every call.
...
Same idea as sticky scratch: **known geometry → reuse pages**, don’t return
multi‑GB regions to the OS between stages or between timed iterations. Stage
buffers allocate **sequentially** (same peak VRAM as before). The benchmark’s
VRAM pre-check was softened so pool-held memory doesn’t look like an OOM skip
after a warm run.

### Measured (Step 1 of the progressive log)

| Stage | Before | After (best of pair) | Δ |
|---|---:|---:|---:|
| A) NTSC | 67.4 | **2.7** | **−96%** |
...
Stage A now matches the isolated kernel. Stage B matches isolated build.
That is the definition of “the wall was alloc/first-touch.”
...
After the pool, Stage B still paid a **scatter/gather bridge** between
frame-major spatial scratch and channel-major bands for TN IIR.

### Channel-outer planar

Planar packing was **frame-major** (`m = f·3 + c`) while bands wanted
**channel-outer** (`m' = c·n + f`). That forced a **permutation** in the
offset table.
...
**Result:** correctness/layout foundation. **Compute ~113 → ~110 ms**—inside
noise.

### Contiguous band write/add

With affine layout, scatter is no longer a scatter—it’s a dense subtract/add
over `M·N` floats. Drop the per-call offset table H2D; use `band_subtract` /
`band_add` into `out_base + level_off`.

**Result:** cleaner code. **Compute ~110 → ~106 ms**—still noise-level.

### Honest read

Both steps were **worth doing for the design**, not for a blog-scale speedup.
They remove a class of “irregular layout tax” so the *next* kernel work isn’t
fighting a permutation. They did **not** beat run-to-run variance the way the
pool did.
...
| Stage C (packaged) | **~18%** | Multi-launch; 1ch IIR ~1.3–1.6× TN copy when clean |

Arithmetic intensity is still ≪ the FP32/HBM ridge. Nothing here is
“needs more FLOPs.”

### Smem fused **downsample** (kept)

Production order is still **cols then rows** (matlabPyrTools). Unlike dense
5×5 global fusion (already a wash/loss earlier), this path:
...
That removes the intermediate **`(H, W/2)` global write/read** between the two
1D passes—the same pattern as OpenCV’s `cuda::pyrDown`, with **L2 binom5 +
...
**Lesson:** “fusion” is not a virtue. **Down** fusion helped; **up** fusion
(same family as the old dense product) hurt—twice. Do not re-attempt up_conv
fusion without a new isolation A/B that beats separable on recon *and* build.
...
| Downsample | **Smem fused** cols→rows |
| Upsample | **Separable** rows→cols (fused tried and failed) |
...
| Start | ~934 | 1.0× |
| TN IIR | ~718 | ~1.3× |
| Sticky scratch | ~377–407 | ~2.3–2.5× |
| DeviceMemPool | ~113–133 | ~7–8× |
| Layout foundation | ~106–110 | ~8.5× |
| Smem fused down | **~96–104** | **~9×** |
...
## What we learned (whole series)

1. **Re-stage the table after every real fix.** After sticky + TN, the next
   “slow stages” were still inflated by **DeviceBuffer** thrash, not 5-tap
   math.
2. **Isolation vs pipeline is a first-class tool.** Kernel @ 93% BW + stage @
   60 ms means measure the allocator, not the matvec.
3. **Same filter, fix layout** beat “invent a new temporal algorithm” for Stage
   C. TN IIR ≈ copy on clean runs.
4. **Layout cleanups can be correct and still free.** Channel-outer +
   contiguous band were good engineering; they were not another 3×.
5. **Fuse the near-roof pass that still pays an intermediate buffer; don’t
   fuse the weak-access product that already lost once.** Down yes; up no.
6. **After ~100 ms compute, transfer owns the wall.** Further mid-pipeline
   work is for purity and paper numbers—not for “real-time file→file” without
...
addresses**, **not throwing multi‑GB scratch away every pyramid call**, and
**not malloc/freeing stage buffers every stage**.
...
| Stage C multi-series IIR / CUDA Graphs | **Open** — kernel can sit near BW; stage is still ~9×3 launches |
| PCIe + encode | **Open** — product path, not mid-pipeline |
| Scan / blocked IIR | Still low ROI while TN IIR ≈ copy on clean runs |
...
~0.9 s of compute because Stage C walked memory uncoalesced and pyramid calls
threw multi‑GB scratch away; coalesced TN IIR and sticky scratch cut that to
~0.4 s, a free-list pool then collapsed the remaining fake stage walls to real
kernel times (~4× again), channel-outer layout cleaned the band bridge without
buying much wall time, smem fused *downsample* shaved a few more milliseconds,
and fused *upsample* regressed again—so production keeps separable up and sits
near ~100 ms mid-pipeline compute for baby.mp4 (~9× from the start of this
series), with file→file now dominated by transfer and encode.**
