...
Every number was measured on the machine named beside it. Timing covers the
magnification only; reading and writing the video are excluded, since those
depend on the codec rather than on this library.

## What is being timed

Four different things get called "how fast is it", and on the same clip they
differ by a factor of six. Every figure below says which one it is.

| Level | What it includes | Use it when |
|---|---|---|
| **Kernels** | The graphics processor's work alone, data already on the card | Comparing against another implementation's kernel time, or embedding this as one stage of a larger device-resident computation |
...
RTX 3090, one motion clip of 301 frames of which the pipelines process 291,
measured 2026-08-18:
...
is about twice what every kernel together costs — and the remaining 78.1 ms is
preparing the input array, checking it, and allocating the output.

Every figure on this page for this card comes from that one session and is
stored in `benches/bench_rtx3090.json`, which
`scripts/dev/record_gpu_bench.py` regenerates. **Compare like with like:** a
kernel figure against another project's wall clock overstates this library by
about six times.
...
life. Three changes account for the difference, all in
[the internals notes](internals/design.md): the temporal filter's running state
moved from 64-bit to 32-bit floating point, which this class of card runs
...
more than it saved. Those two enlargement kernels now run at 92% to 96% of what
the card's memory can sustain, and the shrinking kernel at 94%. Little is left
to win in the kernels; the remaining cost is the transfers above.
...
60-frame clip Metal was faster than OpenCL. The ranking changes with clip length
and would change again on other hardware, which is why the library hard-codes no
preference beyond "native before PyTorch, anything before the processor".
...
Three backends keep up with a 30-frames-per-second camera at 720p. OpenCL,
fastest on whole clips here, is slowest on live frames: batching is where it
wins. **The hand-written NVIDIA backend does not appear because it cannot
stream** — it implements the four whole-clip pipelines but not the
frame-at-a-time operations, and refuses with an explanation rather than failing
partway through a frame. On NVIDIA hardware, PyTorch is the only way to magnify a
live stream today.

## Hand-written against PyTorch, same card

The comparison people ask for. Same RTX 3090, same clip, same entry point, array
in and array out, one backend per process.
...
The hand-written code is 2.5 times faster — the expected result, and the reason
this project exists. That PyTorch is fastest at live streaming is a separate
fact about a different kind of work; neither figure generalises to the other.

*One backend per process matters here.* With both in one process, the NVIDIA
backend's device memory pool starves whatever runs next: measured 2026-08-18,
PyTorch takes 4,003 ms that way against 596 ms alone — 6.7 times slower for no
reason other than the measurement setup.
...
† Measured before the three motion-path changes described above, and not re-run,
so this table mixes two versions of the code. It could not be re-run on
2026-08-22: the cluster partition holding the A100s was down for a reboot.

The other four rows are current, and are four different architectures — which is
what tells you the three changes are not an Ampere trick. Each card was measured
before and after, on its own hardware:
...
◊ On the H100 colour moved 15%. The colour kernels are byte-identical between the
two runs: the only change to those three source files in between is comment text.
So that 15% is the environment, and the older run records neither a date nor a
commit to narrow it further. Motion moved 150%, an order of magnitude outside it.
The run also held one of four GPUs on a shared cluster node, which leaves its
transfer figures looser than its kernel figures.
...
`benches/bench_h100.json` (2026-08-22, the same script on a private HPC cluster
node),
...
slower processor they give a much larger one. The graphics side does not change.
Wherever a ratio appears here, the processor it was measured against is named.
...
**Half precision is not free.** It is faster and uses less memory, and it differs
from single precision by a measurable amount: on the motion pipeline the two
differ by 0.00140 of full scale in root-mean-square error, with a largest single
difference of 2 output levels out of 255 (measured 2026-08-18 on `baby.mp4`,
comparing the 8-bit outputs). The colour pipeline differs by 0.00071 and 1 level.
