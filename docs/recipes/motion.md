...
|---|---|---|
| Two running averages (`motion` preset) | You want a broad band and the simplest behaviour, or you work frame by frame | Cheapest; needs only the previous frame |
...
The overlap exists because the filters need history: without it each chunk
restarts from nothing and the joins show.
